...
export async function POST(req: NextRequest) {
  ...
  const { data: over, error: thrErr } = await db.rpc("pc_auth_throttle_hit", {
    p_token: TOKEN,
    p_ip: ip + ":" + sid,
    p_max: THROTTLE_MAX,
  ...
  if (action === "status") {
    const { data, error } = await db.rpc("pc_student_is_claimed", { p_token: TOKEN, p_id: sid });
  ...
  if (action === "register") {
    ...
    const { data, error } = await db.rpc("pc_student_register", { p_token: TOKEN, p_id: sid, p_hash: full });
    if (error) return NextResponse.json({ ok: false, error: "server" }, { status: 500 });
  ...
  // 3) 로그인 (본인 비번 검증)
  if (action === "login") {
    const { data: salt, error: e1 } = await db.rpc("pc_student_get_salt", { p_token: TOKEN, p_id: sid });
    ...
    const { data: res, error: e2 } = await db.rpc("pc_student_verify", { p_token: TOKEN, p_id: sid, p_full: full });
