...
export async function POST(req: NextRequest) {
  ...
  const { data: claimed, error: cErr } = await db.rpc("pc_student_is_claimed", { p_token: TOKEN, p_id: sid });
  ...
  if (kind === "dwell") {
    ...
    const { error } = await db.rpc("pc_record_dwell", {
      p_token: TOKEN,
      p_id: sid,
  ...
  const { error } = await db.from("pc_lecture_events").insert(row);
  if (error) {
